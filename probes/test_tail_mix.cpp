// Does lane-0 output depend on OTHER lanes' data at fixed lanes=4?
...
int main(int argc, char** argv){
    ...
    const size_t core_dim=6144,H=5120,L=4;
    std::vector<uint16_t> core(core_dim*L), res(H*L), out;
    // lane 0 fixed
    for(size_t c=0;c<core_dim;++c) core[c*L]=rnd16();
    for(size_t c=0;c<H;++c) res[c*L]=rnd16();
    auto run=[&](const char* tag){
        ...
        printf("%s lane0_hash=%llx\n", tag, h);
    };
    run("A");
    // mutate lanes 1..3 only
    for(size_t c=0;c<core_dim;++c) for(size_t l=1;l<L;++l) core[c*L+l]=rnd16();
    for(size_t c=0;c<H;++c) for(size_t l=1;l<L;++l) res[c*L+l]=rnd16();
    run("B");
    return 0;
}
